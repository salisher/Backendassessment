...
$nameErr = $phoneErr = $salaryErr = $classErr = "";

// Fetch classes that do not have a teacher assigned
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Class ID can be left empty if it's a new assignment or select from available classes
    // Validate if class_id is required based on selection
    if (empty($class_id) && $classes_without_teacher->num_rows == 0) {
        $classErr = "All classes have teachers assigned. Class ID is required";
        $valid = false;
    }
